...
const generateMissingPositions = (): THREE.Vector3[] => {
  return Array.from({ length: CUBE_CONFIG.SCATTER_COUNT }, () => 
    new THREE.Vector3(
    ...
    for (let y = 0; y < CUBE_CONFIG.GRID_SIZE; y++) {
      for (let z = 0; z < CUBE_CONFIG.GRID_SIZE; z++) {
        if (!isMissingPosition(x, y, z, missingPositions)) {
          cubes.push(createCube(new THREE.Vector3(
...
const isMissingPosition = (x: number, y: number, z: number, missingPositions: THREE.Vector3[]): boolean => {
  return missingPositions.some(pos => 
...
const createCube = (position: THREE.Vector3, isScattered = false): CubeData => {
  return {
    ...
    float: {
      ...
      amplitude: Math.random() * (CUBE_CONFIG.FLOAT.MAX_AMPLITUDE - CUBE_CONFIG.FLOAT.MIN_AMPLITUDE) + CUBE_CONFIG.FLOAT.MIN_AMPLITUDE,
      rotationAxis: new THREE.Vector3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).normalize(),
      speed: Math.random() * (CUBE_CONFIG.FLOAT.MAX_SPEED - CUBE_CONFIG.FLOAT.MIN_SPEED) + CUBE_CONFIG.FLOAT.MIN_SPEED,
      rotationSpeed: Math.random() * (CUBE_CONFIG.FLOAT.MAX_SPEED - CUBE_CONFIG.FLOAT.MIN_SPEED) + CUBE_CONFIG.FLOAT.MIN_SPEED,
    },
    scatter: {
